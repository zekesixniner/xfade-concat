...
- **Monotonic timestamps at the joins.** Pieces with different B-frame delays
  ...
  `setts`, so the join is clean.
- **Checks.** Every piece is verified for frame count and timestamp delay, the join
  is checked for timestamp warnings, and the output for its total frame count.
